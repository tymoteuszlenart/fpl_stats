...
def build_awards(df, agg, id_to_name, bootstrap_path=DEFAULT_BOOTSTRAP_PATH):
    ...
    add_award(
        "Kapitan? Oczywiście, że trafiłem",
    ...
    add_award(
        "Zawsze wygrywam w drugim sezonie",
    ...
    if not bb.empty:
        best_bb = bb.loc[bb["bench"].idxmax()]
        add_award(
            "Ławka",
            best_bb["entry_name"],
        ...
        if len(bb) > 1:
            worst_bb = bb.loc[bb["bench"].idxmin()]
            add_award(
                "Po co był ten BB?",
                worst_bb["entry_name"],
                f"Najgorsze pojedyncze Bench Boost (GW {int(worst_bb['gw'])})",
                f"{int(worst_bb['bench'])} pkt",
            )

    tc = df[df["chip"].isin(TRIPLE_CAPTAIN_CHIPS)]
    if not tc.empty:
        best_tc = tc.loc[tc["captain_contribution_points"].idxmax()]
        add_award(
            "Haaland na podkurwkę Radka",
            best_tc["entry_name"],
            f"Najlepsze pojedyncze 3xC (GW {int(best_tc['gw'])})",
            f"{int(best_tc['captain_contribution_points'])} pkt",
        )
        if len(tc) > 1:
            worst_tc = tc.loc[tc["captain_contribution_points"].idxmin()]
            add_award(
                "3× i zero zysków",
                worst_tc["entry_name"],
                f"Najgorsze pojedyncze 3xC (GW {int(worst_tc['gw'])})",
                f"{int(worst_tc['captain_contribution_points'])} pkt",
            )

    df["prev_points"] = (
        df.sort_values(["entry_name", "gw"]).groupby("entry_name")["points"].shift(1)
    )
    fh = df[df["chip"].isin(FREE_HIT_CHIPS)]
    if not fh.empty:
        best_fh = fh.loc[fh["points"].idxmax()]
        add_award(
            "Słuchaj mam czutkę!",
            best_fh["entry_name"],
        ...
        if len(fh) > 1:
            worst_fh = fh.loc[fh["points"].idxmin()]
            add_award(
                "Haha słuchaj również mam czutkę!",
                worst_fh["entry_name"],
                f"Najgorsze pojedyncze Free Hit (GW {int(worst_fh['gw'])})",
                f"{int(worst_fh['points'])} pkt",
            )

    all_picked = team_lists_series(df).dropna().explode()
    picked_starting = all_picked[
        all_picked.apply(lambda p: p["multiplier"] > 0 if isinstance(p, dict) else False)
    ]
    ...
    add_award(
        "Pierdolę, każdy gra tego Haalanda",
        top_player_name,
        "Najczęściej wybierany zawodnik",
        f"{top_count} razy",
    )

    try:
        id_to_team, team_id_by_short = load_bootstrap_clubs(bootstrap_path)
        add_season_club_point_awards(df, id_to_team, team_id_by_short, add_award)
    ...
    add_award(
        "Black week? Lajt.",
        min_row["entry_name"],
        f"Najgorszy wynik w jednej kolejce (GW {int(min_row['gw'])})",
        f"{int(min_row['points'])} pkt",
    )
    add_award(
        "Ten GW to nie przypadek",
        max_row["entry_name"],
        f"Najlepszy wynik w jednej kolejce (GW {int(max_row['gw'])})",
        f"{int(max_row['points'])} pkt",
    )
    add_award(
        "Ławka > jedenastka (rekord)",
        bench_max["entry_name"],
        f"Najwięcej punktów na ławce w jednej kolejce (GW {int(bench_max['gw'])})",
        f"{int(bench_max['bench'])} pkt",
    )
    add_award(
        "Ławka: czysto dekoracyjna",
        bench_min["entry_name"],
        f"Najmniej punktów na ławce w jednej kolejce (GW {int(bench_min['gw'])})",
        f"{int(bench_min['bench'])} pkt",
    )

    chip_usage = season_chip_usage_by_entry(df)
    chip_rules = f"(max {MAX_CHIPS_PER_SEASON}: po dwa BB, 3xC, FH i WC w 2025/26)"
    ...
    if not eligible.empty:
        ...
        if (
            worst_eff_idx != best_eff_idx
            or chip_efficiency.loc[worst_eff_idx] < chip_efficiency.loc[best_eff_idx]
        ):
            add_award(
    ...
    top_captains = (
        df.groupby(["entry_name", "captain_id"])["captain_contribution_points"].max().reset_index()
    )
    ...
    top_captain_rows = df.loc[
        idx, ["entry_name", "captain_id", "captain_contribution_points", "gw"]
    ]
    ...
    top_captains["desc"] = (
        ...
        + " – "
        + top_captains["captain_contribution_points"].astype(int).astype(str)
        + " pkt - "
        + "GW"
        + top_captains["gw"].astype(str)
    )
