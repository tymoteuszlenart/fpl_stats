...
def test_bench_boost_award_uses_best_single_not_sum_of_halves():
    ...
    bb = next(a for a in awards if a["Nagroda"] == "Ławka")
    assert bb["Drużyna"] == "A"
    assert bb["Wartość"] == "20 pkt"
    assert "GW 25" in bb["Za co"]
    worst_bb = next(a for a in awards if a["Nagroda"] == "Po co był ten BB?")
    assert worst_bb["Drużyna"] == "A"
    assert worst_bb["Wartość"] == "10 pkt"
    assert "GW 5" in worst_bb["Za co"]
